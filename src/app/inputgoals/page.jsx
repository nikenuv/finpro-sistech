...
const GoalsSelection = () => {
  ...
  const goals = [
    { days: 3, description: 'Baby steps' },
    { days: 7, description: 'Strong start' },
    { days: 14, description: 'Clearly committed' },
    { days: 30, description: 'Unstoppable Streak' },
  ];

  const handleGoalSelection = (days) => {
      ...
      <div className="bg-white p-8 max-w-md w-full">
        ...
        <div className="flex flex-col border-2 p-2 border-black rounded-xl">
          {goals.map((goal) => (
            <button
              key={goal.days}
              onClick={() => handleGoalSelection(goal.days)}
